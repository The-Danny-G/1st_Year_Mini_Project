...
MaBoSS_Comparison<-function(MaBoSS_csv,Named_Stable_States,Probs){
  ...
  PlotData<-data.frame(State=seq(1,n),
                       ...
                       Estimate=c(rep('Mine',n),rep('MaBoSS',n)))
...
start <- Sys.time()
Cohen_Simple_Sims<-FindStableStates(Cohen_Simple,c('ECMicroenv','DNAdamage'),10000,300)
end<-Sys.time()
...
# Then running simulations
start <- Sys.time()
Cohen_Full_Sims<-FindStableStates(Cohen_Full,c('ECMicroenv','DNAdamage'),10000,300)
end <- Sys.time()
